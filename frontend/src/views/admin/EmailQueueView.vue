...
async function loadStats() {
  try {
    ...
    errorMessage.value = e.message || 'Failed to load queue stats';
  }
}
...
async function loadFirstPage() {
  ...
  try {
    ...
    errorMessage.value =
      e.response?.data?.message || e.message || 'Failed to load email queue';
    rows.value = [];
...
async function loadMore() {
  ...
  try {
    ...
    const e = err as { response?: { data?: { message?: string } }; message?: string };
    errorMessage.value =
      e.response?.data?.message || e.message || 'Failed to load more queue entries';
  } finally {
...
async function retryNow(id: number) {
  try {
    ...
    const e = err as { message?: string };
    errorMessage.value = e.message || 'Retry failed';
  }
}
...
async function confirmCancel() {
  ...
  try {
    ...
    const e = err as { message?: string };
    errorMessage.value = e.message || 'Cancel failed';
  }
}
...
<template>
  <div class="flex flex-col gap-6 p-6">
    <header class="flex flex-col gap-2">
      <h1 class="text-2xl font-semibold">Outbound email queue</h1>
      <p class="text-sm text-secondary">
        Durable record of every reply we've tried to send. The worker drains pending rows
        every few seconds; failed sends retry with exponential backoff. Use this view to
        investigate why a notification didn't fire.
      </p>
    </header>

    <section class="grid grid-cols-2 md:grid-cols-4 gap-3">
      <div class="rounded border border-default bg-surface p-3">
        <div class="text-xs text-secondary uppercase tracking-wide">Pending</div>
        <div class="text-2xl font-semibold mt-1">{{ pendingTotal }}</div>
        <div class="text-xs text-secondary mt-1">Oldest: {{ oldestPendingAge }}</div>
      </div>
      <div class="rounded border border-default bg-surface p-3">
        <div class="text-xs text-secondary uppercase tracking-wide">Sent</div>
        <div class="text-2xl font-semibold mt-1">{{ sentTotal }}</div>
      </div>
      <div class="rounded border border-default bg-surface p-3">
        <div class="text-xs text-secondary uppercase tracking-wide">Failed (retrying)</div>
        <div class="text-2xl font-semibold mt-1">{{ failedTotal }}</div>
      </div>
      <div class="rounded border border-default bg-surface p-3">
        <div class="text-xs text-secondary uppercase tracking-wide">Dead (no retry)</div>
    ...
    <EmptyState
      ...
      icon="inbox"
      title="No outbound emails"
      description="Either no replies have been sent recently, or the filters exclude every row."
    />
