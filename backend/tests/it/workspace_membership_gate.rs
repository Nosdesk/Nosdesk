...
fn member_passes_non_member_gets_403() {
    ...
    let (ws, member_uuid, stranger_uuid) = {
        ...
        let ws = crate::common::mint_workspace(&mut conn, "acme-gate", "Acme Gate");
        let member = crate::common::insert_user(&mut conn, "Gate Member");
        let stranger = crate::common::insert_user(&mut conn, "Gate Stranger");
...
fn member_of_another_workspace_is_denied() {
    ...
    let (ws_a, ws_b, user_uuid) = {
        let mut conn = pool.get().expect("conn");
        let ws_a = crate::common::mint_workspace(&mut conn, "tenant-a", "Tenant A");
        let ws_b = crate::common::mint_workspace(&mut conn, "tenant-b", "Tenant B");
        let user = crate::common::insert_user(&mut conn, "A Member");
        (ws_a, ws_b, user.uuid)
    };
