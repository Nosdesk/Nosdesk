...
onboarding-error-token-expired = Deze installatielink is verlopen — hij is slechts ongeveer 30 minuten geldig nadat de server is gestart. Start de server opnieuw op om een nieuwe link te genereren en gebruik die om je beheerdersaccount aan te maken.
...
onboarding-error-token-not-present = Deze installatielink is niet meer geldig — het eenmalige token is slechts ongeveer 30 minuten geldig nadat de server is gestart en wordt daarna gewist. Er bestaat nog geen beheerdersaccount, dus start de server opnieuw op om een nieuwe link te genereren en de installatie af te ronden.
