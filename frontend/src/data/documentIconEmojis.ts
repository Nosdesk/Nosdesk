/**
 * Searchable emoji catalogue for the document icon picker.
 * Kept as plain data so UI components can preload/cache assets
 * without re-parsing a giant inline array on every mount.
 */

export interface DocumentIconEmoji {
  emoji: string
  keywords: string[]
}

export function uniqueEmojis(emojis: string[]): string[] {
  return [...new Set(emojis)]
}

export const documentIconEmojiDatabase: DocumentIconEmoji[] = [
...
export const allDocumentIconEmojis = documentIconEmojiDatabase.map((entry) => entry.emoji)
