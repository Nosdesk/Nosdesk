...
pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.route("/assets", web::get().to(crate::handlers::get_all_devices))
        .route(
            "/assets/paginated",
            web::get().to(crate::handlers::get_paginated_devices),
        )
        .route(
            "/assets/paginated/excluding",
            web::get().to(crate::handlers::get_paginated_devices_excluding),
        )
        .route(
            "/assets/bulk",
            web::post().to(crate::handlers::bulk_devices),
        )
        .route(
            "/assets/calendar-overlay",
            web::get().to(crate::handlers::assets::calendar_overlay),
        )
        .route(
            "/assets/export",
            web::get().to(crate::handlers::export_assets),
        )
        .route(
            "/assets/locations",
            web::get().to(crate::handlers::get_asset_locations),
        )
        .route(
            "/assets/itad-vendors",
            web::get().to(crate::handlers::asset_lifecycle::list_itad_vendors),
        )
        .route(
            "/assets/grouping-dataset",
            web::get().to(crate::handlers::assets::asset_grouping_dataset),
        )
        .route(
            "/assets/rollouts",
            web::post().to(crate::handlers::assets::create_rollout),
        )
        ...
        .route(
            "/asset-kinds",
            web::get().to(crate::handlers::asset_kinds::list_for_picker),
        )
        // Asset model catalog (manufacturers + models). Technician-gated CRUD.
        .route(
            "/manufacturers",
            web::get().to(crate::handlers::manufacturers::list),
        )
        .route(
            "/manufacturers",
            web::post().to(crate::handlers::manufacturers::create),
        )
        .route(
            "/manufacturers/{id:\\d+}",
            web::get().to(crate::handlers::manufacturers::get),
        )
        .route(
            "/manufacturers/{id:\\d+}",
            web::put().to(crate::handlers::manufacturers::update),
        )
        .route(
            "/manufacturers/{id:\\d+}",
            web::delete().to(crate::handlers::manufacturers::delete),
        )
        .route(
            "/asset-models",
            web::get().to(crate::handlers::asset_models::list),
        )
        .route(
            "/asset-models",
            web::post().to(crate::handlers::asset_models::create),
        )
        .route(
            "/asset-models/{id:\\d+}",
            web::get().to(crate::handlers::asset_models::get),
        )
        .route(
            "/asset-models/{id:\\d+}",
            web::put().to(crate::handlers::asset_models::update),
        )
        .route(
            "/asset-models/{id:\\d+}",
            web::delete().to(crate::handlers::asset_models::delete),
        )
        // Native asset groups: picker/list is agent-tier, CRUD is
        // admin-only (see crate::handlers::asset_groups).
        .route(
            "/asset-groups",
            web::get().to(crate::handlers::asset_groups::list_groups),
        )
        .route(
            "/asset-groups",
            web::post().to(crate::handlers::asset_groups::create_group),
        )
        .route(
            "/asset-groups/{id:\\d+}",
            web::put().to(crate::handlers::asset_groups::update_group),
        )
        .route(
            "/asset-groups/{id:\\d+}/archive",
            web::post().to(crate::handlers::asset_groups::archive_group),
        )
        .route(
            "/asset-groups/{id:\\d+}/restore",
            web::post().to(crate::handlers::asset_groups::restore_group),
        )
        .route("/assets", web::post().to(crate::handlers::create_device))
        .route(
            "/assets/empty",
            web::post().to(crate::handlers::create_empty_device),
        )
        .route(
            "/assets/{id:\\d+}/model",
            web::post().to(crate::handlers::set_asset_model),
        )
        .route(
            "/assets/{id:\\d+}/model",
            web::delete().to(crate::handlers::clear_asset_model),
        )
        .route(
            "/assets/{id:\\d+}",
            web::get().to(crate::handlers::get_device_by_id),
        )
        .route(
            "/assets/{id:\\d+}",
            web::put().to(crate::handlers::update_device),
        )
        .route(
            "/assets/{id:\\d+}",
            web::delete().to(crate::handlers::delete_device),
        )
        .route(
            "/assets/{id:\\d+}/unmanage",
            web::post().to(crate::handlers::unmanage_device),
        )
        .route(
            "/assets/{id:\\d+}/lifecycle",
            web::get().to(crate::handlers::asset_lifecycle::list_for_asset),
        )
        .route(
            "/assets/{id:\\d+}/lifecycle",
            web::post().to(crate::handlers::asset_lifecycle::create_transition),
        )
        .route(
            "/assets/{id:\\d+}/disposal",
            web::get().to(crate::handlers::asset_lifecycle::get_disposal),
        )
        .route(
            "/assets/{id:\\d+}/record-card",
            web::get().to(crate::handlers::assets::record_card),
        )
        .route(
            "/assets/{id:\\d+}/loans",
            web::get().to(crate::handlers::asset_loans::list_for_asset),
        )
        .route(
            "/assets/{id:\\d+}/loans",
            web::post().to(crate::handlers::asset_loans::issue),
        )
        .route(
            "/assets/{id:\\d+}/loans/{loan_id:\\d+}",
            web::patch().to(crate::handlers::asset_loans::edit),
        )
        .route(
            "/assets/{id:\\d+}/loans/{loan_id:\\d+}/return",
            web::post().to(crate::handlers::asset_loans::return_loan),
        )
        .route(
            "/assets/{id:\\d+}/media",
            web::get().to(crate::handlers::asset_media::list_for_asset),
        )
        .route(
            "/assets/{id:\\d+}/media",
            web::post().to(crate::handlers::asset_media::upload_for_asset),
        )
        .route(
            "/assets/{id:\\d+}/media/{media_id}",
            web::put().to(crate::handlers::asset_media::update_media),
        )
        .route(
            "/assets/{id:\\d+}/media/{media_id}",
            web::delete().to(crate::handlers::asset_media::delete_media),
        )
        .route(
            "/assets/{id:\\d+}/usage",
            web::post().to(crate::handlers::asset_usage::record),
        )
        .route(
            "/assets/{id:\\d+}/usage",
            web::get().to(crate::handlers::asset_usage::list_for_asset),
        )
        .route(
            "/assets/{id:\\d+}/audit",
            web::post().to(crate::handlers::asset_audits::record),
        )
        .route(
            "/assets/{id:\\d+}/audits",
            web::get().to(crate::handlers::asset_audits::list_for_asset),
        )
        // Native asset-group membership, assigned from the asset side.
        .route(
            "/assets/{id:\\d+}/groups",
            web::put().to(crate::handlers::asset_groups::set_asset_groups),
        )
        .route(
            "/users/{uuid}/assets",
            web::get().to(crate::handlers::get_user_devices),
        );
}
