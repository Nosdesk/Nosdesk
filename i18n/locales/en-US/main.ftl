...
nav-expand-sidebar = Expand sidebar
nav-collapse-sidebar = Collapse sidebar
nav-search-with-shortcut = Search ({ $shortcut })
...
accept-invitation-heading-added = { $inviter } added you to { $workspace }
accept-invitation-heading-workspace = You've been added to { $workspace }
...
accept-invitation-subheading-guest-confirm = Confirm your email address to send your request to the team.
accept-invitation-submit-guest-confirm = Confirm my request
accept-invitation-confirmed-guest = Your request is with the team. Replies will arrive by email.
accept-invitation-subheading-invitation = Set a password and you're in.
...
accept-invitation-submit-invitation = Set password and sign in
...
email-mode-heading = How this workspace sends
email-mode-subtitle = Choose the address and server your helpdesk mail goes out through. Each option keeps its settings, so you can switch back at any time.
email-mode-in-use = In use
email-mode-default-title = Server default
email-mode-default-body = The SMTP server set in this server's environment.
email-mode-default-body-managed = Nosdesk sends from your workspace's built-in address.
email-mode-domain-title = Your domain
email-mode-domain-body = Send from your own address through the server's SMTP, signed with DKIM.
email-mode-domain-body-managed = Send from your own address through Nosdesk, signed with DKIM.
email-mode-relay-title = Your SMTP server
email-mode-relay-body = Send through Microsoft 365, Google Workspace, Amazon SES or any SMTP server.
email-mode-error-switch = Could not switch how this workspace sends.
email-mode-cancel = Cancel
email-default-env-note = Set by the server's operator in its environment. Change these in the .env file or Docker environment and restart.
email-default-managed-note = Mail goes out from your workspace's built-in address, delivered by Nosdesk. Customers can reply to it directly.
email-default-not-configured = The server has no SMTP settings, so nothing sends in this mode. Set the variables below, or choose another option.
email-default-env-vars = Environment variables
email-default-password-set = Set
email-default-password-not-set = Not set
email-default-use-button = Use the server default
email-default-now-active = This workspace now sends with the server default.
email-domain-setup-intro-self-hosted = Enter the address your helpdesk should send from. Mail goes through the server's SMTP settings, signed for your domain, and you publish one DNS record.
email-domain-use-button = Send from this domain
email-domain-now-active = This workspace now sends from its domain.
email-domain-pending-fallback-note = Until the domain is verified, mail goes out with the server default.
email-domain-error-from-moved = The From address is no longer on this domain. Set the domain up again to send from it.
email-domain-remove-confirm-title = Remove the sending domain?
email-domain-remove-confirm-message = Its DKIM key is deleted and the workspace goes back to the server default. Setting the domain up again needs a new DNS record.
email-relay-provider-label = Provider
email-relay-preset-other = Other
email-relay-preset-m365-help = The mailbox needs SMTP AUTH turned on. Microsoft turns off password sign-in for SMTP by default from the end of 2026, so check your tenant still allows it.
email-relay-preset-google-help = Sign in with the account's address and an app password (needs 2-Step Verification). For Google's IP-allowlisted relay, use smtp-relay.gmail.com and leave the username blank.
email-relay-preset-ses-help = Use your region's endpoint and the SMTP credentials from the SES console (not your AWS access keys). The From domain must be verified in SES.
email-relay-preset-mailgun-help = Use the SMTP credentials from your domain's settings. Domains in the EU region use smtp.eu.mailgun.org.
email-relay-preset-postmark-help = Use a server API token as both the username and the password.
email-relay-host-label = Server
email-relay-port-label = Port
email-relay-security-label = Security
email-relay-security-none = None
email-relay-username-label = Username
email-relay-password-label = Password
email-relay-password-stored = Stored. Leave blank to keep it.
email-relay-password-remove = Remove stored password
email-relay-password-removed = Stored password removed.
email-relay-no-username-warning = Blank sends without signing in. Your server must accept mail from this server's address.
email-relay-from-help = Your SMTP server must allow sending as this address.
email-relay-port-465-needs-tls = Port 465 needs TLS. Use 587 for STARTTLS.
email-relay-port-587-needs-starttls = Port 587 needs STARTTLS. Use 465 for TLS.
email-relay-port-25-warning = Port 25 is for server-to-server relay and is often blocked. Prefer 587 or 465.
email-relay-plaintext-warning = Without security, the password and mail cross the network unencrypted. Use only for a relay on a trusted network.
email-relay-test-and-use = Test and use
email-relay-test-and-save = Test and save
email-relay-test-only = Send test only
email-relay-test-note = Tests send one message to you.
email-relay-save-anyway = Save anyway
email-relay-saved = SMTP server saved.
email-relay-now-active = This workspace now sends through your SMTP server.
email-relay-error-save = Could not save the SMTP server.
email-relay-error-host = Enter a server name like smtp.example.com, without a port or https://.
email-relay-error-port = Enter a port between 1 and 65535.
email-relay-error-port-security = This port and security setting do not work together.
email-relay-error-from = Enter a valid From address.
email-relay-error-password-required = Enter the password. The stored one is only used for the saved server and username.
email-relay-error-rate-limited = Too many tests. Wait a few minutes and try again.
email-relay-dns-description = Live check of the From domain. Its SPF record should include your provider.
email-test-heading = Test sending
email-test-description = Sends a test message to you with whatever this workspace uses now.
email-test-ok-title = Accepted by the server
email-test-ok-hint = Check { $to } for the test message. Accepted means the server took it; it can still take a minute to arrive, or land in spam.
email-test-dns-title = Server not found
email-test-dns-hint = Check the server name for typos.
email-test-egress-blocked-title = Connection not allowed
email-test-egress-blocked-hint = Private and internal addresses are blocked. The server's operator can allow a host with NOSDESK_OUTBOUND_ALLOWED_HOSTS.
email-test-egress-blocked-hosted-hint = Private and internal addresses are blocked. Use your provider's public server name.
email-test-connect-title = Could not connect
email-test-connect-hint = Check the port, and that outbound connections on it are allowed.
email-test-tls-title = Secure connection failed
email-test-tls-hint = Check the security setting matches the port: STARTTLS on 587, TLS on 465.
email-test-auth-title = Sign-in refused
email-test-auth-hint = Check the username and password. Many providers need an app password, or SMTP sign-in turned on for the mailbox.
email-test-rejected-title = Message refused
email-test-rejected-hint = The server may not allow sending from this From address.
email-test-timeout-title = No answer in time
email-test-timeout-hint = Check the port, and any firewall between the two servers.
email-test-incomplete-title = Nothing to send with
email-test-incomplete-hint = This workspace has no working sending setup yet.
email-test-invalid-title = Settings incomplete
email-test-invalid-hint = Check the fields above.
email-test-unknown-title = The test failed
email-test-unknown-hint = The server's reply is below.

imap-preset-google-help = Turn on IMAP in Gmail's settings, then sign in with an app password (needs 2-Step Verification).
imap-preset-m365-help = Microsoft 365 no longer accepts passwords over IMAP, so its mailboxes can't be checked this way. Forward the mailbox to a workspace address instead.
imap-preset-m365-forwarding-link = Set up forwarding
imap-test-ok-title = Signed in
imap-test-ok-hint = The server accepted the sign-in and the mailbox opened.
imap-test-dns-title = Server not found
imap-test-dns-hint = Check the server name for typos.
imap-test-egress-blocked-title = Connection not allowed
imap-test-connect-title = Could not connect
imap-test-connect-hint = Check the port (usually 993), and that outbound connections on it are allowed.
imap-test-tls-title = Secure connection failed
imap-test-tls-hint = The server must offer TLS on this port. Most use 993.
imap-test-auth-title = Sign-in refused
imap-test-auth-hint = Check the username and password. Many providers need an app password, and IMAP turned on for the mailbox.
imap-test-mailbox-title = Mailbox not found
imap-test-mailbox-hint = Check the mailbox name. Most servers use INBOX.
imap-test-timeout-title = No answer in time
imap-test-timeout-hint = Check the port, and any firewall between the two servers.
imap-test-invalid-title = Settings incomplete
imap-test-invalid-hint = Check the fields above.
imap-test-unknown-title = The test failed
imap-test-unknown-hint = The server's reply is below.
...
admin-workspaces-community-cap = This self-hosted deployment is limited to { $max } workspace. A license lifts the limit.
...
workspace-members-manage-in-control-plane = Manage staff on your Nosdesk account
workspace-members-staff-role-hint = Their role is a seat in your Nosdesk account. Change it there.
...
admin-guest-toggle-tickets-description = Shows a public ticket form at { $url }.
...
admin-guest-toggle-public-docs-description = Exposes pages marked 'public' at { $url } without requiring login.
...
admin-guest-toggle-help-description = A help page at { $url } with links to password reset and ticket submission.
...
admin-guest-intro-message-placeholder = e.g. We reply within one business day, Monday to Friday. For urgent outages, call 555-1234.
admin-guest-intro-message-hint = Shown above the public submit form. A good place to say how soon you reply. Plain text, line breaks preserved.
...
user-mgmt-hosted-add-in-control-plane = Staff are added on your Nosdesk account, under Workspaces and then Staff.
...
user-profile-action-manage-in-control-plane = Manage in Nosdesk account
...
settings-notifications-type-ticket-referenced-name = Ticket mentioned
settings-notifications-type-ticket-referenced-description = When a ticket assigned to you is mentioned in a comment
...
ticket-comments-placeholder-public = Reply to the requester... (@ to mention, # for a ticket)
ticket-comments-placeholder-internal = Add an internal note... (@ to mention, # for a ticket)
...
ticket-comments-link-suggestion = Link { $ticket } as related?
ticket-comments-link-suggestion-accept = Link
ticket-comments-link-suggestion-dismiss = Dismiss
...
ticket-activity-phrase-referenced = mentioned this ticket in
ticket-activity-phrase-referenced-internal = mentioned this ticket in an internal note on
...
guest-submit-privacy-note = We only use your email to reply about this request. Nothing you send here is made public.
guest-submit-title-counter = { $count } / { $max }
...
guest-submit-error-title-short = Add a few more words so we know what this is about.
...
editor-ticket-picker-prompt = Type a ticket number or title
editor-ticket-picker-empty = No tickets found
...
editor-embed-render-failed = Couldn't display this page
...
common-n-selected = { $count } selected
common-actions-menu-aria = Actions
common-sheet-aria = Sheet
common-panel-close = Close { $title }
pagination-controls-page-size-all = All
pagination-controls-page-input-aria = Page number
inline-edit-placeholder = Enter text
assignment-picker-search-placeholder = Search users and groups
docs-collection-tree-aria = Pages
date-picker-calendar-aria = Calendar

# Admin: License (LicenseCloudView). Self-hosted licence, Nosdesk Cloud push
# relay, offline instance id.
admin-nav-license-title = License
admin-nav-license-description = License status, the Nosdesk Cloud push relay, and plans.
route-title-admin-license = License
admin-license-title = License
admin-license-description = What this server is licensed for, and how push notifications reach the mobile app.
admin-license-error-load = Failed to load license status
admin-license-community-title = Community edition
admin-license-community-body = Everything on this server works without a license: email in and out, push with your own credentials, and one workspace.
admin-license-licensed-subtitle = Self-hosted license
admin-license-pill-none = No license
admin-license-pill-active = Active
admin-license-pill-expired = Expired
admin-license-pill-invalid = Not valid
admin-license-pill-expiring = { $days ->
    [0] Expires today
    [one] Expires in 1 day
   *[other] Expires in { $days } days
}
admin-license-fact-workspaces = Workspaces
admin-license-fact-workspaces-value = { $used } of { $max }
admin-license-fact-expires = Expires
admin-license-fact-expired = Expired
admin-license-fact-source = Installed
admin-license-source-env = From NOSDESK_LICENSE_KEY
admin-license-source-pasted = Pasted by an admin
admin-license-source-linked = Connected to Nosdesk Cloud
admin-license-expired-body = This license has expired. Existing workspaces keep running, new ones are capped at one, and push through Nosdesk Cloud stops. Renewing issues a new key to paste here.
admin-license-expiring-body = Renewals install themselves while this server can reach Nosdesk Cloud. If it cannot, paste the new key from your account before this one expires.
admin-license-open-account = Open your Nosdesk account
admin-license-env-managed = Set by NOSDESK_LICENSE_KEY. Change it in the environment and restart.
admin-license-replace = Replace key
admin-license-remove = Remove license
admin-license-remove-confirm-title = Remove the license?
admin-license-remove-confirm-body = This server returns to the Community edition. Existing workspaces keep running, but no new ones can be created past the first, and push through Nosdesk Cloud stops. You can paste the key again at any time.
admin-license-installed = License installed.
admin-license-removed = License removed.
admin-license-error-install = The license could not be installed. Try again.
admin-license-error-remove = The license could not be removed. Try again.
admin-license-reason-malformed = That is not a license key. Keys start with nsk_lic_ and are one long line.
admin-license-reason-unknown-key = This key was signed by a key this version of Nosdesk does not know. Update Nosdesk, then try again.
admin-license-reason-bad-signature = The signature on this key does not match. Copy it again from your Nosdesk account; a single changed character breaks it.
admin-license-reason-wrong-issuer = This key was not issued by Nosdesk.
admin-license-reason-invalid-claims = This key is missing details a license needs. Issue a new one from your Nosdesk account.
admin-license-reason-expired = This key has expired. Your Nosdesk account has the current one.
admin-license-reason-unreadable = A license key is stored but this server can no longer decrypt it, usually because the encryption key changed. Paste the key again.
admin-license-compare-title = What a license adds
admin-license-compare-feature = Feature
admin-license-compare-now = This server now
admin-license-compare-licensed = With a license
admin-license-compare-email = Email in and out
admin-license-compare-email-both = Your SMTP and IMAP
admin-license-compare-workspaces = Workspaces
admin-license-compare-workspaces-licensed = As many as your plan includes
admin-license-compare-push = Push to the mobile app
admin-license-compare-push-now = Your own app build and credentials
admin-license-compare-push-licensed = The Nosdesk app, through Nosdesk Cloud
admin-license-compare-support = Support
admin-license-compare-support-now = Community
admin-license-compare-support-licensed = Email support
admin-license-pricing = Plans and pricing
admin-license-paste-title = Install a license key
admin-license-paste-title-replace = Replace the license key
admin-license-paste-label = License key
admin-license-paste-help = Keys start with nsk_lic_. This server checks the signature itself before saving it.
admin-license-paste-submit = Install
admin-license-paste-where = Where do I find my key?
admin-license-cancel = Cancel
admin-license-push-title = Mobile push
admin-license-push-body = How notifications reach the Nosdesk mobile app. Your own credentials only reach your own build of the app; the app from the App Store and Google Play is reached through Nosdesk Cloud.
admin-license-push-mode-default = Automatic
admin-license-push-mode-native = Your credentials
admin-license-push-mode-relay = Nosdesk Cloud
admin-license-push-mode-off = Off
admin-license-push-env-managed = Set by NOSDESK_PUSH_MODE ({ $mode }). Change it in the environment and restart.
admin-license-push-error-unavailable = Push could not switch to that mode. The server log says why.
admin-license-push-error-save = The push mode could not be saved. Try again.
admin-license-push-native-ok = Sending with this server's credentials
admin-license-push-native-missing = No APNs or FCM credentials are set on this server, so nothing is sent in this mode.
admin-license-push-off = Push is off
admin-license-push-none = No push credentials on this server
admin-license-push-none-hint = Choose Nosdesk Cloud (needs a license), or set APNs and FCM credentials for your own build of the app.
admin-license-push-relay-disclosure = In this mode the server sends its license, its instance ID, device tokens and each notification as it appears on the phone to Nosdesk Cloud, which passes them to Apple and Google without keeping the device tokens.
admin-license-relay-ok = Connected to Nosdesk Cloud
admin-license-relay-idle = Not used yet
admin-license-relay-no-license = Needs a license
admin-license-relay-no-license-hint = Install a license above to send push through Nosdesk Cloud.
admin-license-relay-dpa = Data processing agreement not accepted
admin-license-relay-dpa-hint = Accept it on your Nosdesk account, then check again.
admin-license-relay-rejected = Nosdesk Cloud did not accept this license
admin-license-relay-rejected-hint = It may have been replaced or cancelled. Paste the current key from your account.
admin-license-relay-cap = Over this period's usage
admin-license-relay-cap-hint = Push resumes when the next period starts.
admin-license-relay-throttled = Briefly rate limited
admin-license-relay-unreachable = Cannot reach Nosdesk Cloud
admin-license-relay-unreachable-hint = Check that this server can make outbound HTTPS requests to api.nosdesk.com.
admin-license-relay-unexpected = Unexpected answer from Nosdesk Cloud
admin-license-relay-last-success = Last worked { $when }
admin-license-relay-retry = Check again
admin-license-docs = Read the setup guide
admin-license-offline-title = Server without internet access
admin-license-offline-body = A license is checked on this server and never needs to reach Nosdesk, so buy one from your Nosdesk account on any machine and paste the key above. Quote this instance ID if you contact support.
admin-license-connect = Connect to Nosdesk Cloud
admin-license-paste-instead = Have a key? Paste it instead
admin-license-connect-code-label = Your code
admin-license-connect-open = Open Nosdesk to approve
admin-license-connect-help = Or go to { $url } on any device and enter the code. Sign in, choose a plan if you have none, and approve this server.
admin-license-connect-waiting = Waiting for approval. The code expires in { $time }.
admin-license-connect-denied = The code was refused on your Nosdesk account. Nothing was installed.
admin-license-connect-expired = The code expired before it was approved.
admin-license-connect-failed = Nosdesk Cloud sent a license this server could not install. Try again, or paste the key.
admin-license-connect-again = Try again
admin-license-connect-done = Connected. The license is installed.
...
admin-license-sync-unreachable = Could not check for renewals. This license keeps working until it expires.
admin-license-sync-updated = A renewed license is installed.
admin-license-sync-current = This license is current.
admin-workspaces-community-cap-link = Manage license

# Customer portal
portal-sign-in-title = Sign in to { $app }
portal-sign-in-intro = Enter your email and we'll send you a sign-in link.
portal-sign-in-email-label = Email
portal-sign-in-submit = Send sign-in link
portal-sign-in-sent = If we have your email on file, a sign-in link is on its way. Check your inbox.
...
portal-filter-closed = Closed
portal-requests-empty-title = No requests yet
portal-requests-empty-closed = No closed requests
portal-requests-empty-description = When you ask the team for help, your requests show up here.
portal-requests-load-failed = We couldn't load your requests. Try again in a moment.
portal-updated = Updated { $when }
portal-request-number = Request #{ $id }
portal-opened = Opened { $when }
portal-thread-staff = Support team
portal-thread-you = You
portal-reply-label = Reply
...
portal-new-subject-placeholder = A short summary of what you need
portal-new-description-label = Description
portal-new-description-placeholder = Tell us what's going on and how we can help.
...
# Identity owned by the Nosdesk account (hosted staff)
nosdesk-account-title = Nosdesk account
nosdesk-account-profile-self = Your name, photo and email address are managed in your Nosdesk account.
nosdesk-account-profile-other = Their name, photo and email address are managed in their Nosdesk account.
nosdesk-account-security-self = Your password, two-step verification and passkeys are managed in your Nosdesk account.
nosdesk-account-security-other = Their sign-in is managed in their Nosdesk account.
nosdesk-account-name = Name
nosdesk-account-email = Email
nosdesk-account-open-self = Edit in your Nosdesk account
nosdesk-account-open-other = Manage in Nosdesk account
nosdesk-account-chip = Nosdesk account
nosdesk-account-chip-tooltip = Name, photo, email and sign-in are managed in their Nosdesk account.
workspace-name-title = Name in this workspace
workspace-name-description = Shown to people in this workspace instead of your account name. Leave it empty to use your account name.
