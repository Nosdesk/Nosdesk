...
/// Mint an api_token row directly. `is_platform_scoped` controlled
/// by the caller so a single test can hold both kinds.
fn mint_token(
...
async fn workspaces_create_full_contract() {
    ...
    let platform_token = mint_token(&mut pool.get().expect("conn"), &owner, "ctrl-plane", true);
    let user_token = mint_token(&mut pool.get().expect("conn"), &owner, "user", false);
