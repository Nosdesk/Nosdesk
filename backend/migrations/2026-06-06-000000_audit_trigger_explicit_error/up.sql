-- Layer 2: make a missing audit context fail loudly.
--
-- audit_log.workspace_id is NOT NULL and defaults from the
-- app.workspace_id GUC. A write to an audited table that runs outside
-- with_actor_context() / with_actor_bypass_context() leaves the GUC
-- unset, the default resolves to NULL, and the insert fails with an
-- opaque NOT NULL violation on a monthly partition the caller has
-- never heard of. Raise a typed, named error instead so the remedy
-- ("wrap the call") is obvious from the message.
--
-- This rebuilds the 2026-05-26-120000 function verbatim (audit-read
-- guard + column redaction unchanged) and only inserts one guard after
-- the audit-read short-circuit. ERRCODE NDX01 is Nosdesk-specific
-- ("you forgot the actor-context wrapper"); see the doc comment on
-- crate::sync::session::with_actor_context.
