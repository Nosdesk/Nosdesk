...
describe('TimePicker', () => {
  ...
  it('renders a label tied to the field and puts error on the segments', async () => {
    ...
    expect(wrapper.get('[role="group"]').attributes('aria-describedby')).toBeUndefined()
    await wrapper.setProps({ error: 'Outside opening hours' })
    const group = wrapper.get('[role="group"]')
    expect(wrapper.get(`#${group.attributes('aria-describedby')}`).text()).toBe('Outside opening hours')
  })
})
