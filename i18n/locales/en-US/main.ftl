...
onboarding-error-token-expired = This setup link has expired — it's only valid for about 30 minutes after the server starts. Restart the server to generate a fresh link, then use the new one to finish creating your admin account.
onboarding-error-token-mismatch = The setup token is incorrect. Check the token in the server startup logs and try again.
onboarding-error-token-not-present = This setup link is no longer valid — the one-time token is only valid for about 30 minutes after the server starts, then it's cleared. No admin account exists yet, so restart the server to generate a fresh link and finish setup.
