...
async fn session_jwt_authenticates_as_bearer() {
    ...
    let user = crate::common::insert_user(&mut pool.get().expect("conn"), "Bearer Alice");
...
async fn revoked_session_rejects_bearer() {
    ...
    let db = crate::common::TestDb::new();
    let pool = db.pool_with_size(2);
    let user = crate::common::insert_user(&mut pool.get().expect("conn"), "Bearer Bob");
...
async fn sse_scope_jwt_rejected_on_bearer() {
    ...
    let db = crate::common::TestDb::new();
    let pool = db.pool_with_size(2);
    let user = crate::common::insert_user(&mut pool.get().expect("conn"), "Bearer Carol");
...
async fn nsk_api_token_still_authenticates() {
    ...
    let db = crate::common::TestDb::new();
    let pool = db.pool_with_size(2);
    let user = crate::common::insert_user(&mut pool.get().expect("conn"), "Bearer Dave");
    let api_token =
        crate::common::mint_api_token(&mut pool.get().expect("conn"), &user, "regression");
