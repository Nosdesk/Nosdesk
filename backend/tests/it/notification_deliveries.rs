...
fn rows_are_written_once_and_in_app_starts_delivered() {
    ...
    let (ws, user) = {
        ...
        (
            crate::common::mint_workspace(&mut conn, "deliv", "Deliveries"),
            crate::common::insert_user(&mut conn, "deliv-recipient").uuid,
        )
    };
...
fn a_failed_attempt_reschedules_then_fails_when_spent() {
    ...
    let (ws, user) = {
        let mut conn = pool.get().expect("conn");
        (
            crate::common::mint_workspace(&mut conn, "deliv2", "Deliveries 2"),
            crate::common::insert_user(&mut conn, "deliv-recipient-2").uuid,
        )
    };
