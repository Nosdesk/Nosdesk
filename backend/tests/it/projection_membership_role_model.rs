...
fn provisioning_membership_persists_under_nosdesk_app_role() {
    ...
    let ws_id = crate::common::mint_workspace(&mut conn, "rolecheck", "Role Check");
    let user = crate::common::insert_user(&mut conn, "Owner Person");
...
fn membership_gate_needs_workspace_scope_to_see_the_row() {
    ...
    let mut conn = pool.get().expect("conn");
    let ws_id = crate::common::mint_workspace(&mut conn, "gatecheck", "Gate Check");
    let user = crate::common::insert_user(&mut conn, "Member Person");
