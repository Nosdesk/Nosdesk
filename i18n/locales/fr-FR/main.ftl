...
# MACHINE TRANSLATION, pending native review
accept-invitation-heading-added = { $inviter } vous a ajouté à { $workspace }
accept-invitation-heading-workspace = Vous avez été ajouté à { $workspace }
...
accept-invitation-subheading-guest-confirm = Confirmez votre adresse e-mail pour transmettre votre demande à l'équipe.
accept-invitation-submit-guest-confirm = Confirmer ma demande
accept-invitation-confirmed-guest = Votre demande a été transmise à l'équipe. Les réponses vous parviendront par e-mail.
accept-invitation-subheading-invitation = Choisissez un mot de passe et c'est parti.
...
accept-invitation-submit-invitation = Définir le mot de passe et se connecter
...
email-mode-heading = Mode d'envoi de cet espace de travail
email-mode-subtitle = Choisissez l'adresse et le serveur utilisés pour les e-mails de votre helpdesk. Chaque option conserve ses réglages, vous pouvez donc revenir en arrière à tout moment.
email-mode-in-use = Utilisé
email-mode-default-title = Réglage du serveur
email-mode-default-body = Le serveur SMTP défini dans l'environnement de ce serveur.
email-mode-default-body-managed = Nosdesk envoie depuis l'adresse intégrée de votre espace de travail.
email-mode-domain-title = Votre domaine
email-mode-domain-body = Envoyez depuis votre propre adresse via le SMTP du serveur, avec une signature DKIM.
email-mode-domain-body-managed = Envoyez depuis votre propre adresse via Nosdesk, avec une signature DKIM.
email-mode-relay-title = Votre serveur SMTP
email-mode-relay-body = Envoyez via Microsoft 365, Google Workspace, Amazon SES ou tout autre serveur SMTP.
email-mode-error-switch = Impossible de changer le mode d'envoi de cet espace de travail.
email-mode-cancel = Annuler
email-default-env-note = Défini par l'opérateur du serveur dans son environnement. Modifiez-les dans le fichier .env ou l'environnement Docker, puis redémarrez.
...
email-domain-use-button = Envoyer depuis ce domaine
email-domain-now-active = Cet espace de travail envoie désormais depuis son domaine.
email-domain-pending-fallback-note = Tant que le domaine n'est pas vérifié, les e-mails partent avec le réglage du serveur.
email-domain-error-from-moved = L'adresse d'expédition n'est plus sur ce domaine. Configurez à nouveau le domaine pour envoyer depuis celui-ci.
email-domain-remove-confirm-title = Supprimer le domaine d'envoi ?
email-domain-remove-confirm-message = Sa clé DKIM est supprimée et l'espace de travail revient au réglage du serveur. Reconfigurer le domaine nécessitera un nouvel enregistrement DNS.
email-relay-provider-label = Fournisseur
email-relay-preset-other = Autre
email-relay-preset-m365-help = La boîte aux lettres doit avoir SMTP AUTH activé. Microsoft désactive par défaut la connexion SMTP par mot de passe à partir de fin 2026 : vérifiez que votre tenant l'autorise encore.
email-relay-preset-google-help = Connectez-vous avec l'adresse du compte et un mot de passe d'application (validation en deux étapes requise). Pour le relais Google autorisé par IP, utilisez smtp-relay.gmail.com et laissez le nom d'utilisateur vide.
email-relay-preset-ses-help = Utilisez le point de terminaison de votre région et les identifiants SMTP de la console SES (pas vos clés d'accès AWS). Le domaine d'expédition doit être vérifié dans SES.
email-relay-preset-mailgun-help = Utilisez les identifiants SMTP des réglages de votre domaine. Les domaines de la région UE utilisent smtp.eu.mailgun.org.
email-relay-preset-postmark-help = Utilisez un jeton d'API de serveur comme nom d'utilisateur et comme mot de passe.
email-relay-host-label = Serveur
email-relay-port-label = Port
email-relay-security-label = Sécurité
email-relay-security-none = Aucune
email-relay-username-label = Nom d'utilisateur
email-relay-password-label = Mot de passe
email-relay-password-stored = Enregistré. Laissez vide pour le conserver.
email-relay-password-remove = Supprimer le mot de passe enregistré
email-relay-password-removed = Mot de passe enregistré supprimé.
email-relay-no-username-warning = Vide : l'envoi se fait sans connexion. Votre serveur doit accepter les e-mails provenant de l'adresse de ce serveur.
email-relay-from-help = Votre serveur SMTP doit autoriser l'envoi avec cette adresse.
email-relay-port-465-needs-tls = Le port 465 nécessite TLS. Utilisez 587 pour STARTTLS.
email-relay-port-587-needs-starttls = Le port 587 nécessite STARTTLS. Utilisez 465 pour TLS.
email-relay-port-25-warning = Le port 25 sert au relais entre serveurs et est souvent bloqué. Préférez 587 ou 465.
email-relay-plaintext-warning = Sans sécurité, le mot de passe et les e-mails circulent sans chiffrement. À utiliser uniquement pour un relais sur un réseau de confiance.
email-relay-test-and-use = Tester et utiliser
email-relay-test-and-save = Tester et enregistrer
email-relay-test-only = Envoyer un test seulement
email-relay-test-note = Les tests vous envoient un message.
email-relay-save-anyway = Enregistrer quand même
email-relay-saved = Serveur SMTP enregistré.
email-relay-now-active = Cet espace de travail envoie désormais via votre serveur SMTP.
email-relay-error-save = Impossible d'enregistrer le serveur SMTP.
email-relay-error-host = Saisissez un nom de serveur comme smtp.example.com, sans port ni https://.
email-relay-error-port = Saisissez un port entre 1 et 65535.
email-relay-error-port-security = Ce port et ce réglage de sécurité ne fonctionnent pas ensemble.
email-relay-error-from = Saisissez une adresse d'expédition valide.
email-relay-error-password-required = Saisissez le mot de passe. Celui enregistré n'est utilisé que pour le serveur et le nom d'utilisateur enregistrés.
email-relay-error-rate-limited = Trop de tests. Patientez quelques minutes et réessayez.
email-relay-dns-description = Vérification en direct du domaine d'expédition. Son enregistrement SPF doit inclure votre fournisseur.
email-test-heading = Tester l'envoi
email-test-description = Vous envoie un message de test avec le mode actuellement utilisé par cet espace de travail.
email-test-ok-title = Accepté par le serveur
email-test-ok-hint = Vérifiez { $to } pour le message de test. Accepté signifie que le serveur l'a pris en charge ; il peut encore mettre une minute à arriver ou finir dans les spams.
email-test-dns-title = Serveur introuvable
email-test-dns-hint = Vérifiez le nom du serveur.
email-test-egress-blocked-title = Connexion non autorisée
email-test-egress-blocked-hint = Les adresses privées et internes sont bloquées. L'opérateur du serveur peut autoriser un hôte avec NOSDESK_OUTBOUND_ALLOWED_HOSTS.
email-test-egress-blocked-hosted-hint = Les adresses privées et internes sont bloquées. Utilisez le nom de serveur public de votre fournisseur.
email-test-connect-title = Connexion impossible
email-test-connect-hint = Vérifiez le port et que les connexions sortantes sur ce port sont autorisées.
email-test-tls-title = Échec de la connexion sécurisée
email-test-tls-hint = Vérifiez que la sécurité correspond au port : STARTTLS sur 587, TLS sur 465.
email-test-auth-title = Connexion refusée
email-test-auth-hint = Vérifiez le nom d'utilisateur et le mot de passe. De nombreux fournisseurs exigent un mot de passe d'application ou l'activation de la connexion SMTP pour la boîte aux lettres.
email-test-rejected-title = Message refusé
email-test-rejected-hint = Le serveur n'autorise peut-être pas l'envoi depuis cette adresse d'expédition.
email-test-timeout-title = Pas de réponse à temps
email-test-timeout-hint = Vérifiez le port et tout pare-feu entre les deux serveurs.
email-test-incomplete-title = Aucun moyen d'envoi
email-test-incomplete-hint = Cet espace de travail n'a pas encore de configuration d'envoi fonctionnelle.
email-test-invalid-title = Réglages incomplets
email-test-invalid-hint = Vérifiez les champs ci-dessus.
email-test-unknown-title = Le test a échoué
email-test-unknown-hint = La réponse du serveur figure ci-dessous.

imap-preset-google-help = Activez IMAP dans les paramètres de Gmail, puis connectez-vous avec un mot de passe d'application (validation en deux étapes requise).
imap-preset-m365-help = Microsoft 365 n'accepte plus les mots de passe via IMAP : ses boîtes aux lettres ne peuvent donc pas être relevées ainsi. Transférez plutôt la boîte vers une adresse de l'espace de travail.
imap-preset-m365-forwarding-link = Configurer le transfert
imap-test-ok-title = Connecté
imap-test-ok-hint = Le serveur a accepté la connexion et la boîte aux lettres s'est ouverte.
imap-test-dns-title = Serveur introuvable
imap-test-dns-hint = Vérifiez le nom du serveur.
imap-test-egress-blocked-title = Connexion non autorisée
imap-test-connect-title = Connexion impossible
imap-test-connect-hint = Vérifiez le port (généralement 993) et que les connexions sortantes sur ce port sont autorisées.
imap-test-tls-title = Échec de la connexion sécurisée
imap-test-tls-hint = Le serveur doit proposer TLS sur ce port. La plupart utilisent 993.
imap-test-auth-title = Connexion refusée
imap-test-auth-hint = Vérifiez le nom d'utilisateur et le mot de passe. De nombreux fournisseurs exigent un mot de passe d'application et l'activation d'IMAP pour la boîte aux lettres.
imap-test-mailbox-title = Boîte aux lettres introuvable
imap-test-mailbox-hint = Vérifiez le nom de la boîte aux lettres. La plupart des serveurs utilisent INBOX.
imap-test-timeout-title = Pas de réponse à temps
imap-test-timeout-hint = Vérifiez le port et tout pare-feu entre les deux serveurs.
imap-test-invalid-title = Réglages incomplets
imap-test-invalid-hint = Vérifiez les champs ci-dessus.
imap-test-unknown-title = Le test a échoué
imap-test-unknown-hint = La réponse du serveur figure ci-dessous.
...
admin-workspaces-community-cap = Ce déploiement auto-hébergé est limité à { $max } espace de travail. Une licence lève cette limite.
...
admin-guest-toggle-tickets-description = Affiche un formulaire public de ticket à { $url }.
...
admin-guest-toggle-public-docs-description = Rend accessibles sans connexion les pages marquées « publiques » à { $url }.
...
admin-guest-toggle-help-description = Une page d'aide à { $url } avec des liens vers la réinitialisation du mot de passe et l'envoi de tickets.
...
admin-guest-intro-message-placeholder = ex. Nous répondons sous un jour ouvré, du lundi au vendredi. Pour les pannes urgentes, appelez le 555-1234.
admin-guest-intro-message-hint = Affiché au-dessus du formulaire public. Idéal pour indiquer votre délai de réponse. Texte brut, retours à la ligne conservés.
...
# MACHINE TRANSLATION, pending native review
date-picker-open-calendar-aria = Ouvrir le calendrier
date-segment-day = Jour
date-segment-month = Mois
date-segment-year = Année
date-segment-hour = Heure
date-segment-minute = Minute
date-segment-second = Seconde
date-segment-day-period = AM/PM
date-range-start-segment = { $segment } de début
date-range-end-segment = { $segment } de fin
...
guest-submit-privacy-note = Nous utilisons votre e-mail uniquement pour répondre à cette demande. Rien de ce que vous envoyez ici n'est rendu public.
...
guest-submit-error-title-short = Ajoutez quelques mots de plus pour que nous sachions de quoi il s'agit.
...
# Admin : Licence (LicenseCloudView).
# MACHINE TRANSLATION, pending native review
admin-nav-license-title = Licence
admin-nav-license-description = État de la licence, relais push Nosdesk Cloud et offres.
route-title-admin-license = Licence
admin-license-title = Licence
admin-license-description = Ce que couvre la licence de ce serveur, et comment les notifications push atteignent l'application mobile.
admin-license-error-load = Impossible de charger l'état de la licence
admin-license-community-title = Édition Community
admin-license-community-body = Tout fonctionne sur ce serveur sans licence : e-mails entrants et sortants, push avec vos propres identifiants, et un espace de travail.
admin-license-licensed-subtitle = Licence auto-hébergée
admin-license-pill-none = Aucune licence
admin-license-pill-active = Active
admin-license-pill-expired = Expirée
admin-license-pill-invalid = Non valide
admin-license-pill-expiring = { $days ->
    [0] Expire aujourd'hui
    [one] Expire dans 1 jour
   *[other] Expire dans { $days } jours
}
admin-license-fact-workspaces = Espaces de travail
admin-license-fact-workspaces-value = { $used } sur { $max }
admin-license-fact-expires = Expire le
admin-license-fact-expired = Expirée le
admin-license-fact-source = Installée
admin-license-source-env = Depuis NOSDESK_LICENSE_KEY
admin-license-source-pasted = Collée par un administrateur
admin-license-source-linked = Connectée à Nosdesk Cloud
admin-license-expired-body = Cette licence a expiré. Les espaces de travail existants continuent de fonctionner, les nouveaux sont limités à un, et le push via Nosdesk Cloud s'arrête. Le renouvellement émet une nouvelle clé à coller ici.
admin-license-expiring-body = Les renouvellements s'installent d'eux-mêmes tant que ce serveur peut joindre Nosdesk Cloud. Sinon, collez la nouvelle clé depuis votre compte avant l'expiration de celle-ci.
admin-license-open-account = Ouvrir votre compte Nosdesk
admin-license-env-managed = Définie par NOSDESK_LICENSE_KEY. Modifiez-la dans l'environnement puis redémarrez.
admin-license-replace = Remplacer la clé
admin-license-remove = Retirer la licence
admin-license-remove-confirm-title = Retirer la licence ?
admin-license-remove-confirm-body = Ce serveur repasse en édition Community. Les espaces de travail existants continuent de fonctionner, mais aucun nouveau ne peut être créé au-delà du premier, et le push via Nosdesk Cloud s'arrête. Vous pouvez recoller la clé à tout moment.
admin-license-installed = Licence installée.
admin-license-removed = Licence retirée.
admin-license-error-install = La licence n'a pas pu être installée. Réessayez.
admin-license-error-remove = La licence n'a pas pu être retirée. Réessayez.
admin-license-reason-malformed = Ce n'est pas une clé de licence. Les clés commencent par nsk_lic_ et tiennent sur une seule longue ligne.
admin-license-reason-unknown-key = Cette clé a été signée par une clé que cette version de Nosdesk ne connaît pas. Mettez Nosdesk à jour, puis réessayez.
admin-license-reason-bad-signature = La signature de cette clé ne correspond pas. Copiez-la à nouveau depuis votre compte Nosdesk ; un seul caractère modifié la rend invalide.
admin-license-reason-wrong-issuer = Cette clé n'a pas été émise par Nosdesk.
admin-license-reason-invalid-claims = Il manque à cette clé des informations nécessaires à une licence. Émettez-en une nouvelle depuis votre compte Nosdesk.
admin-license-reason-expired = Cette clé a expiré. Votre compte Nosdesk contient la clé actuelle.
admin-license-reason-unreadable = Une clé de licence est enregistrée, mais ce serveur ne peut plus la déchiffrer, généralement parce que la clé de chiffrement a changé. Collez la clé à nouveau.
admin-license-compare-title = Ce qu'apporte une licence
admin-license-compare-feature = Fonction
admin-license-compare-now = Ce serveur aujourd'hui
admin-license-compare-licensed = Avec une licence
admin-license-compare-email = E-mails entrants et sortants
admin-license-compare-email-both = Votre SMTP et votre IMAP
admin-license-compare-workspaces = Espaces de travail
admin-license-compare-workspaces-licensed = Autant que votre offre en inclut
admin-license-compare-push = Push vers l'application mobile
admin-license-compare-push-now = Votre propre build de l'application et vos identifiants
admin-license-compare-push-licensed = L'application Nosdesk, via Nosdesk Cloud
admin-license-compare-support = Support
admin-license-compare-support-now = Communauté
admin-license-compare-support-licensed = Support par e-mail
admin-license-pricing = Offres et tarifs
admin-license-paste-title = Installer une clé de licence
admin-license-paste-title-replace = Remplacer la clé de licence
admin-license-paste-label = Clé de licence
admin-license-paste-help = Les clés commencent par nsk_lic_. Ce serveur vérifie lui-même la signature avant de l'enregistrer.
admin-license-paste-submit = Installer
admin-license-paste-where = Où trouver ma clé ?
admin-license-cancel = Annuler
admin-license-push-title = Push mobile
admin-license-push-body = Comment les notifications atteignent l'application mobile Nosdesk. Vos propres identifiants n'atteignent que votre propre build de l'application ; l'application de l'App Store et de Google Play est atteinte via Nosdesk Cloud.
admin-license-push-mode-default = Automatique
admin-license-push-mode-native = Vos identifiants
admin-license-push-mode-relay = Nosdesk Cloud
admin-license-push-mode-off = Désactivé
admin-license-push-env-managed = Défini par NOSDESK_PUSH_MODE ({ $mode }). Modifiez-le dans l'environnement puis redémarrez.
admin-license-push-error-unavailable = Le push n'a pas pu passer à ce mode. Le journal du serveur indique pourquoi.
admin-license-push-error-save = Le mode push n'a pas pu être enregistré. Réessayez.
admin-license-push-native-ok = Envoi avec les identifiants de ce serveur
admin-license-push-native-missing = Aucun identifiant APNs ou FCM n'est défini sur ce serveur ; rien n'est envoyé dans ce mode.
admin-license-push-off = Le push est désactivé
admin-license-push-none = Aucun identifiant push sur ce serveur
admin-license-push-none-hint = Choisissez Nosdesk Cloud (nécessite une licence), ou définissez des identifiants APNs et FCM pour votre propre build de l'application.
admin-license-push-relay-disclosure = Dans ce mode, le serveur envoie à Nosdesk Cloud sa licence, son identifiant d'instance, les jetons des appareils et chaque notification telle qu'elle s'affiche sur le téléphone ; Nosdesk Cloud les transmet à Apple et Google sans conserver les jetons des appareils.
admin-license-relay-ok = Connecté à Nosdesk Cloud
admin-license-relay-idle = Pas encore utilisé
admin-license-relay-no-license = Licence requise
admin-license-relay-no-license-hint = Installez une licence ci-dessus pour envoyer le push via Nosdesk Cloud.
admin-license-relay-dpa = Accord de traitement des données non accepté
admin-license-relay-dpa-hint = Acceptez-le sur votre compte Nosdesk, puis vérifiez à nouveau.
admin-license-relay-rejected = Nosdesk Cloud n'a pas accepté cette licence
admin-license-relay-rejected-hint = Elle a peut-être été remplacée ou annulée. Collez la clé actuelle depuis votre compte.
admin-license-relay-cap = Utilisation de la période dépassée
admin-license-relay-cap-hint = Le push reprend au début de la période suivante.
admin-license-relay-throttled = Brièvement limité
admin-license-relay-unreachable = Impossible de joindre Nosdesk Cloud
admin-license-relay-unreachable-hint = Vérifiez que ce serveur peut effectuer des requêtes HTTPS sortantes vers api.nosdesk.com.
admin-license-relay-unexpected = Réponse inattendue de Nosdesk Cloud
admin-license-relay-last-success = Dernier succès { $when }
admin-license-relay-retry = Vérifier à nouveau
admin-license-docs = Lire le guide d'installation
admin-license-offline-title = Serveur sans accès à Internet
admin-license-offline-body = Une licence est vérifiée sur ce serveur et n'a jamais besoin de joindre Nosdesk : achetez-la depuis votre compte Nosdesk sur n'importe quelle machine et collez la clé ci-dessus. Indiquez cet identifiant d'instance si vous contactez le support.
admin-license-connect = Se connecter à Nosdesk Cloud
admin-license-paste-instead = Vous avez une clé ? Collez-la plutôt
admin-license-connect-code-label = Votre code
admin-license-connect-open = Ouvrir Nosdesk pour approuver
admin-license-connect-help = Ou rendez-vous sur { $url } depuis n'importe quel appareil et saisissez le code. Connectez-vous, choisissez une offre si vous n'en avez pas, puis approuvez ce serveur.
admin-license-connect-waiting = En attente d'approbation. Le code expire dans { $time }.
admin-license-connect-denied = Le code a été refusé sur votre compte Nosdesk. Rien n'a été installé.
admin-license-connect-expired = Le code a expiré avant d'être approuvé.
admin-license-connect-failed = Nosdesk Cloud a envoyé une licence que ce serveur n'a pas pu installer. Réessayez, ou collez la clé.
admin-license-connect-again = Réessayer
admin-license-connect-done = Connecté. La licence est installée.
...
admin-license-sync-unreachable = Impossible de vérifier les renouvellements. Cette licence reste valable jusqu'à son expiration.
admin-license-sync-updated = Une licence renouvelée est installée.
admin-license-sync-current = Cette licence est à jour.
admin-workspaces-community-cap-link = Gérer la licence
...
portal-filter-closed = Clôturées
portal-requests-empty-title = Aucune demande pour le moment
portal-requests-empty-closed = Aucune demande clôturée
portal-requests-empty-description = Lorsque vous demandez de l'aide à l'équipe, vos demandes apparaissent ici.
portal-requests-load-failed = Impossible de charger vos demandes. Réessayez dans un instant.
portal-updated = Mise à jour { $when }
portal-request-number = Demande n° { $id }
portal-opened = Ouverte { $when }
portal-thread-staff = Équipe d'assistance
portal-thread-you = Vous
portal-reply-label = Répondre
...
portal-new-subject-placeholder = Un bref résumé de votre besoin
portal-new-description-label = Description
portal-new-description-placeholder = Dites-nous ce qui se passe et comment nous pouvons vous aider.
