...
notif-ticket-created-requester = [{ $app }] We hebben je verzoek ontvangen: { $title }
...
# MACHINE TRANSLATION, pending native review
accept-invitation-heading-added = { $inviter } heeft je toegevoegd aan { $workspace }
accept-invitation-heading-workspace = Je bent toegevoegd aan { $workspace }
...
accept-invitation-subheading-guest-confirm = Bevestig je e-mailadres om je verzoek naar het team te sturen.
accept-invitation-submit-guest-confirm = Mijn verzoek bevestigen
accept-invitation-confirmed-guest = Je verzoek is bij het team. Antwoorden ontvang je per e-mail.
accept-invitation-subheading-invitation = Kies een wachtwoord en je bent binnen.
...
accept-invitation-submit-invitation = Wachtwoord instellen en inloggen
...
email-mode-heading = Hoe deze werkruimte verzendt
email-mode-subtitle = Kies het adres en de server waarmee de e-mail van je helpdesk wordt verzonden. Elke optie bewaart haar instellingen, dus je kunt altijd terugschakelen.
email-mode-in-use = In gebruik
email-mode-default-title = Serverstandaard
email-mode-default-body = De SMTP-server die in de omgeving van deze server is ingesteld.
email-mode-default-body-managed = Nosdesk verzendt vanaf het ingebouwde adres van je werkruimte.
email-mode-domain-title = Je domein
email-mode-domain-body = Verzend vanaf je eigen adres via de SMTP van de server, ondertekend met DKIM.
email-mode-domain-body-managed = Verzend vanaf je eigen adres via Nosdesk, ondertekend met DKIM.
email-mode-relay-title = Je SMTP-server
email-mode-relay-body = Verzend via Microsoft 365, Google Workspace, Amazon SES of een andere SMTP-server.
email-mode-error-switch = Kon de verzendwijze van deze werkruimte niet wijzigen.
email-mode-cancel = Annuleren
email-default-env-note = Ingesteld door de beheerder van de server in de omgeving. Wijzig ze in het .env-bestand of de Docker-omgeving en herstart.
...
email-domain-use-button = Verzenden vanaf dit domein
email-domain-now-active = Deze werkruimte verzendt nu vanaf haar domein.
email-domain-pending-fallback-note = Totdat het domein is geverifieerd, wordt e-mail verzonden met de serverstandaard.
email-domain-error-from-moved = Het afzenderadres staat niet meer op dit domein. Stel het domein opnieuw in om ervan te verzenden.
email-domain-remove-confirm-title = Verzenddomein verwijderen?
email-domain-remove-confirm-message = De DKIM-sleutel wordt verwijderd en de werkruimte gaat terug naar de serverstandaard. Het domein opnieuw instellen vereist een nieuw DNS-record.
email-relay-provider-label = Provider
email-relay-preset-other = Anders
email-relay-preset-m365-help = De mailbox moet SMTP AUTH ingeschakeld hebben. Microsoft schakelt aanmelden met wachtwoord voor SMTP vanaf eind 2026 standaard uit, dus controleer of je tenant het nog toestaat.
email-relay-preset-google-help = Meld je aan met het adres van het account en een app-wachtwoord (vereist verificatie in twee stappen). Gebruik voor Googles relay op basis van IP-toelating smtp-relay.gmail.com en laat de gebruikersnaam leeg.
email-relay-preset-ses-help = Gebruik het endpoint van je regio en de SMTP-gegevens uit de SES-console (niet je AWS-toegangssleutels). Het afzenderdomein moet in SES geverifieerd zijn.
email-relay-preset-mailgun-help = Gebruik de SMTP-gegevens uit de instellingen van je domein. Domeinen in de EU-regio gebruiken smtp.eu.mailgun.org.
email-relay-preset-postmark-help = Gebruik een server-API-token als gebruikersnaam én als wachtwoord.
email-relay-host-label = Server
email-relay-port-label = Poort
email-relay-security-label = Beveiliging
email-relay-security-none = Geen
email-relay-username-label = Gebruikersnaam
email-relay-password-label = Wachtwoord
email-relay-password-stored = Opgeslagen. Laat leeg om het te behouden.
email-relay-password-remove = Opgeslagen wachtwoord verwijderen
email-relay-password-removed = Opgeslagen wachtwoord verwijderd.
email-relay-no-username-warning = Leeg verzendt zonder aan te melden. Je server moet e-mail van het adres van deze server accepteren.
email-relay-from-help = Je SMTP-server moet verzenden als dit adres toestaan.
email-relay-port-465-needs-tls = Poort 465 vereist TLS. Gebruik 587 voor STARTTLS.
email-relay-port-587-needs-starttls = Poort 587 vereist STARTTLS. Gebruik 465 voor TLS.
email-relay-port-25-warning = Poort 25 is voor relay tussen servers en wordt vaak geblokkeerd. Gebruik liever 587 of 465.
email-relay-plaintext-warning = Zonder beveiliging gaan het wachtwoord en de e-mail onversleuteld over het netwerk. Gebruik dit alleen voor een relay op een vertrouwd netwerk.
email-relay-test-and-use = Testen en gebruiken
email-relay-test-and-save = Testen en opslaan
email-relay-test-only = Alleen test verzenden
email-relay-test-note = Tests sturen één bericht naar jou.
email-relay-save-anyway = Toch opslaan
email-relay-saved = SMTP-server opgeslagen.
email-relay-now-active = Deze werkruimte verzendt nu via je SMTP-server.
email-relay-error-save = Kon de SMTP-server niet opslaan.
email-relay-error-host = Voer een servernaam in zoals smtp.example.com, zonder poort of https://.
email-relay-error-port = Voer een poort in tussen 1 en 65535.
email-relay-error-port-security = Deze poort en beveiligingsinstelling werken niet samen.
email-relay-error-from = Voer een geldig afzenderadres in.
email-relay-error-password-required = Voer het wachtwoord in. Het opgeslagen wachtwoord wordt alleen gebruikt voor de opgeslagen server en gebruikersnaam.
email-relay-error-rate-limited = Te veel tests. Wacht een paar minuten en probeer het opnieuw.
email-relay-dns-description = Live controle van het afzenderdomein. Het SPF-record moet je provider bevatten.
email-test-heading = Verzenden testen
email-test-description = Stuurt je een testbericht met wat deze werkruimte nu gebruikt.
email-test-ok-title = Geaccepteerd door de server
email-test-ok-hint = Controleer { $to } op het testbericht. Geaccepteerd betekent dat de server het heeft aangenomen; het kan nog een minuut duren of in spam terechtkomen.
email-test-dns-title = Server niet gevonden
email-test-dns-hint = Controleer de servernaam op typfouten.
email-test-egress-blocked-title = Verbinding niet toegestaan
email-test-egress-blocked-hint = Privé- en interne adressen worden geblokkeerd. De beheerder van de server kan een host toestaan met NOSDESK_OUTBOUND_ALLOWED_HOSTS.
email-test-egress-blocked-hosted-hint = Privé- en interne adressen worden geblokkeerd. Gebruik de openbare servernaam van je provider.
email-test-connect-title = Kon geen verbinding maken
email-test-connect-hint = Controleer de poort en of uitgaande verbindingen daarop zijn toegestaan.
email-test-tls-title = Beveiligde verbinding mislukt
email-test-tls-hint = Controleer of de beveiliging bij de poort past: STARTTLS op 587, TLS op 465.
email-test-auth-title = Aanmelden geweigerd
email-test-auth-hint = Controleer de gebruikersnaam en het wachtwoord. Veel providers vereisen een app-wachtwoord of ingeschakeld SMTP-aanmelden voor de mailbox.
email-test-rejected-title = Bericht geweigerd
email-test-rejected-hint = De server staat verzenden vanaf dit afzenderadres mogelijk niet toe.
email-test-timeout-title = Geen antwoord op tijd
email-test-timeout-hint = Controleer de poort en eventuele firewalls tussen de twee servers.
email-test-incomplete-title = Niets om mee te verzenden
email-test-incomplete-hint = Deze werkruimte heeft nog geen werkende verzendinstelling.
email-test-invalid-title = Instellingen onvolledig
email-test-invalid-hint = Controleer de velden hierboven.
email-test-unknown-title = De test is mislukt
email-test-unknown-hint = Het antwoord van de server staat hieronder.

imap-preset-google-help = Schakel IMAP in bij de Gmail-instellingen en meld je daarna aan met een app-wachtwoord (vereist verificatie in twee stappen).
imap-preset-m365-help = Microsoft 365 accepteert geen wachtwoorden meer via IMAP, dus de mailboxen kunnen zo niet worden opgehaald. Stuur de mailbox in plaats daarvan door naar een werkruimteadres.
imap-preset-m365-forwarding-link = Doorsturen instellen
imap-test-ok-title = Aangemeld
imap-test-ok-hint = De server accepteerde de aanmelding en de mailbox is geopend.
imap-test-dns-title = Server niet gevonden
imap-test-dns-hint = Controleer de servernaam op typfouten.
imap-test-egress-blocked-title = Verbinding niet toegestaan
imap-test-connect-title = Kon geen verbinding maken
imap-test-connect-hint = Controleer de poort (meestal 993) en of uitgaande verbindingen daarop zijn toegestaan.
imap-test-tls-title = Beveiligde verbinding mislukt
imap-test-tls-hint = De server moet TLS aanbieden op deze poort. De meeste gebruiken 993.
imap-test-auth-title = Aanmelden geweigerd
imap-test-auth-hint = Controleer de gebruikersnaam en het wachtwoord. Veel providers vereisen een app-wachtwoord en ingeschakelde IMAP voor de mailbox.
imap-test-mailbox-title = Mailbox niet gevonden
imap-test-mailbox-hint = Controleer de mailboxnaam. De meeste servers gebruiken INBOX.
imap-test-timeout-title = Geen antwoord op tijd
imap-test-timeout-hint = Controleer de poort en eventuele firewalls tussen de twee servers.
imap-test-invalid-title = Instellingen onvolledig
imap-test-invalid-hint = Controleer de velden hierboven.
imap-test-unknown-title = De test is mislukt
imap-test-unknown-hint = Het antwoord van de server staat hieronder.
...
admin-workspaces-community-cap = Deze zelf-gehoste installatie is beperkt tot { $max } werkruimte. Een licentie heft deze limiet op.
...
admin-guest-toggle-tickets-description = Toont een openbaar ticketformulier op { $url }.
...
admin-guest-toggle-public-docs-description = Maakt pagina's die als 'openbaar' zijn gemarkeerd zonder inloggen beschikbaar op { $url }.
...
admin-guest-toggle-help-description = Een helppagina op { $url } met links naar wachtwoordherstel en het indienen van tickets.
...
admin-guest-intro-message-placeholder = bijv. We reageren binnen één werkdag, maandag tot en met vrijdag. Bel bij dringende storingen 555-1234.
admin-guest-intro-message-hint = Wordt boven het openbare formulier getoond. Een goede plek om te zeggen hoe snel je reageert. Platte tekst, regeleinden blijven behouden.
...
settings-emails-source-manual = Handmatig toegevoegd
settings-emails-source-microsoft = Microsoft
settings-emails-source-ldap = LDAP
settings-emails-source-sso = Eenmalige aanmelding
...
guest-submit-privacy-note = We gebruiken je e-mailadres alleen om op deze aanvraag te reageren. Niets wat je hier verstuurt wordt openbaar gemaakt.
...
guest-submit-error-title-short = Voeg nog een paar woorden toe zodat we weten waar het over gaat.
...
editor-embed-render-failed = Kan deze pagina niet weergeven
...
# Beheer: Licentie (LicenseCloudView).
# MACHINE TRANSLATION, pending native review
admin-nav-license-title = Licentie
admin-nav-license-description = Licentiestatus, de pushrelay van Nosdesk Cloud en abonnementen.
route-title-admin-license = Licentie
admin-license-title = Licentie
admin-license-description = Waarvoor deze server een licentie heeft, en hoe pushmeldingen de mobiele app bereiken.
admin-license-error-load = Licentiestatus kon niet worden geladen
admin-license-community-title = Community-editie
admin-license-community-body = Alles op deze server werkt zonder licentie: e-mail in en uit, push met je eigen inloggegevens, en één werkruimte.
admin-license-licensed-subtitle = Zelf-gehoste licentie
admin-license-pill-none = Geen licentie
admin-license-pill-active = Actief
admin-license-pill-expired = Verlopen
admin-license-pill-invalid = Ongeldig
admin-license-pill-expiring = { $days ->
    [0] Verloopt vandaag
    [one] Verloopt over 1 dag
   *[other] Verloopt over { $days } dagen
}
admin-license-fact-workspaces = Werkruimtes
admin-license-fact-workspaces-value = { $used } van { $max }
admin-license-fact-expires = Verloopt
admin-license-fact-expired = Verlopen
admin-license-fact-source = Geïnstalleerd
admin-license-source-env = Uit NOSDESK_LICENSE_KEY
admin-license-source-pasted = Geplakt door een beheerder
admin-license-source-linked = Gekoppeld aan Nosdesk Cloud
admin-license-expired-body = Deze licentie is verlopen. Bestaande werkruimtes blijven werken, nieuwe zijn beperkt tot één, en push via Nosdesk Cloud stopt. Verlengen levert een nieuwe sleutel op om hier te plakken.
admin-license-expiring-body = Verlengingen installeren zichzelf zolang deze server Nosdesk Cloud kan bereiken. Lukt dat niet, plak dan de nieuwe sleutel uit je account voordat deze verloopt.
admin-license-open-account = Open je Nosdesk-account
admin-license-env-managed = Ingesteld via NOSDESK_LICENSE_KEY. Wijzig het in de omgeving en herstart.
admin-license-replace = Sleutel vervangen
admin-license-remove = Licentie verwijderen
admin-license-remove-confirm-title = Licentie verwijderen?
admin-license-remove-confirm-body = Deze server keert terug naar de Community-editie. Bestaande werkruimtes blijven werken, maar na de eerste kunnen er geen nieuwe worden aangemaakt, en push via Nosdesk Cloud stopt. Je kunt de sleutel altijd opnieuw plakken.
admin-license-installed = Licentie geïnstalleerd.
admin-license-removed = Licentie verwijderd.
admin-license-error-install = De licentie kon niet worden geïnstalleerd. Probeer het opnieuw.
admin-license-error-remove = De licentie kon niet worden verwijderd. Probeer het opnieuw.
admin-license-reason-malformed = Dit is geen licentiesleutel. Sleutels beginnen met nsk_lic_ en staan op één lange regel.
admin-license-reason-unknown-key = Deze sleutel is ondertekend met een sleutel die deze versie van Nosdesk niet kent. Werk Nosdesk bij en probeer het opnieuw.
admin-license-reason-bad-signature = De handtekening van deze sleutel klopt niet. Kopieer hem opnieuw uit je Nosdesk-account; één gewijzigd teken maakt hem ongeldig.
admin-license-reason-wrong-issuer = Deze sleutel is niet door Nosdesk uitgegeven.
admin-license-reason-invalid-claims = Deze sleutel mist gegevens die een licentie nodig heeft. Geef een nieuwe uit vanuit je Nosdesk-account.
admin-license-reason-expired = Deze sleutel is verlopen. Je Nosdesk-account bevat de huidige.
admin-license-reason-unreadable = Er is een licentiesleutel opgeslagen, maar deze server kan hem niet meer ontsleutelen, meestal omdat de versleutelingssleutel is gewijzigd. Plak de sleutel opnieuw.
admin-license-compare-title = Wat een licentie toevoegt
admin-license-compare-feature = Functie
admin-license-compare-now = Deze server nu
admin-license-compare-licensed = Met een licentie
admin-license-compare-email = E-mail in en uit
admin-license-compare-email-both = Je eigen SMTP en IMAP
admin-license-compare-workspaces = Werkruimtes
admin-license-compare-workspaces-licensed = Zoveel als je abonnement bevat
admin-license-compare-push = Push naar de mobiele app
admin-license-compare-push-now = Je eigen build van de app en inloggegevens
admin-license-compare-push-licensed = De Nosdesk-app, via Nosdesk Cloud
admin-license-compare-support = Ondersteuning
admin-license-compare-support-now = Community
admin-license-compare-support-licensed = Ondersteuning per e-mail
admin-license-pricing = Abonnementen en prijzen
admin-license-paste-title = Licentiesleutel installeren
admin-license-paste-title-replace = Licentiesleutel vervangen
admin-license-paste-label = Licentiesleutel
admin-license-paste-help = Sleutels beginnen met nsk_lic_. Deze server controleert de handtekening zelf voordat hij wordt opgeslagen.
admin-license-paste-submit = Installeren
admin-license-paste-where = Waar vind ik mijn sleutel?
admin-license-cancel = Annuleren
admin-license-push-title = Mobiele push
admin-license-push-body = Hoe meldingen de mobiele Nosdesk-app bereiken. Je eigen inloggegevens bereiken alleen je eigen build van de app; de app uit de App Store en Google Play wordt via Nosdesk Cloud bereikt.
admin-license-push-mode-default = Automatisch
admin-license-push-mode-native = Je inloggegevens
admin-license-push-mode-relay = Nosdesk Cloud
admin-license-push-mode-off = Uit
admin-license-push-env-managed = Ingesteld via NOSDESK_PUSH_MODE ({ $mode }). Wijzig het in de omgeving en herstart.
admin-license-push-error-unavailable = Push kon niet naar die modus overschakelen. Het serverlog vermeldt waarom.
admin-license-push-error-save = De pushmodus kon niet worden opgeslagen. Probeer het opnieuw.
admin-license-push-native-ok = Verstuurt met de inloggegevens van deze server
admin-license-push-native-missing = Er zijn geen APNs- of FCM-inloggegevens ingesteld op deze server, dus in deze modus wordt niets verstuurd.
admin-license-push-off = Push staat uit
admin-license-push-none = Geen push-inloggegevens op deze server
admin-license-push-none-hint = Kies Nosdesk Cloud (vereist een licentie), of stel APNs- en FCM-inloggegevens in voor je eigen build van de app.
admin-license-push-relay-disclosure = In deze modus stuurt de server zijn licentie, zijn instantie-ID, apparaattokens en elke melding zoals die op de telefoon verschijnt naar Nosdesk Cloud, dat ze doorgeeft aan Apple en Google zonder de apparaattokens te bewaren.
admin-license-relay-ok = Verbonden met Nosdesk Cloud
admin-license-relay-idle = Nog niet gebruikt
admin-license-relay-no-license = Licentie vereist
admin-license-relay-no-license-hint = Installeer hierboven een licentie om push via Nosdesk Cloud te versturen.
admin-license-relay-dpa = Verwerkersovereenkomst niet geaccepteerd
admin-license-relay-dpa-hint = Accepteer die in je Nosdesk-account en controleer daarna opnieuw.
admin-license-relay-rejected = Nosdesk Cloud heeft deze licentie niet geaccepteerd
admin-license-relay-rejected-hint = Mogelijk is hij vervangen of opgezegd. Plak de huidige sleutel uit je account.
admin-license-relay-cap = Gebruik voor deze periode overschreden
admin-license-relay-cap-hint = Push gaat verder zodra de volgende periode begint.
admin-license-relay-throttled = Tijdelijk beperkt
admin-license-relay-unreachable = Nosdesk Cloud is niet bereikbaar
admin-license-relay-unreachable-hint = Controleer of deze server uitgaande HTTPS-verzoeken naar api.nosdesk.com kan doen.
admin-license-relay-unexpected = Onverwacht antwoord van Nosdesk Cloud
admin-license-relay-last-success = Laatst gelukt { $when }
admin-license-relay-retry = Opnieuw controleren
admin-license-docs = Lees de installatiehandleiding
admin-license-offline-title = Server zonder internettoegang
admin-license-offline-body = Een licentie wordt op deze server gecontroleerd en hoeft Nosdesk nooit te bereiken: koop er een via je Nosdesk-account op elke willekeurige computer en plak de sleutel hierboven. Vermeld deze instantie-ID als je contact opneemt met ondersteuning.
admin-license-connect = Verbinden met Nosdesk Cloud
admin-license-paste-instead = Heb je een sleutel? Plak die in plaats daarvan
admin-license-connect-code-label = Je code
admin-license-connect-open = Open Nosdesk om goed te keuren
admin-license-connect-help = Of ga op een willekeurig apparaat naar { $url } en voer de code in. Log in, kies een abonnement als je er geen hebt, en keur deze server goed.
admin-license-connect-waiting = Wachten op goedkeuring. De code verloopt over { $time }.
admin-license-connect-denied = De code is geweigerd in je Nosdesk-account. Er is niets geïnstalleerd.
admin-license-connect-expired = De code is verlopen voordat hij werd goedgekeurd.
admin-license-connect-failed = Nosdesk Cloud stuurde een licentie die deze server niet kon installeren. Probeer het opnieuw, of plak de sleutel.
admin-license-connect-again = Opnieuw proberen
admin-license-connect-done = Verbonden. De licentie is geïnstalleerd.
...
admin-license-sync-unreachable = Kon niet controleren op verlengingen. Deze licentie blijft werken tot hij verloopt.
admin-license-sync-updated = Er is een verlengde licentie geïnstalleerd.
admin-license-sync-current = Deze licentie is actueel.
admin-workspaces-community-cap-link = Licentie beheren
...
portal-filter-closed = Gesloten
portal-requests-empty-title = Nog geen verzoeken
portal-requests-empty-closed = Geen gesloten verzoeken
portal-requests-empty-description = Als je het team om hulp vraagt, verschijnen je verzoeken hier.
portal-requests-load-failed = We konden je verzoeken niet laden. Probeer het zo opnieuw.
portal-updated = Bijgewerkt { $when }
portal-request-number = Verzoek #{ $id }
portal-opened = Geopend { $when }
portal-thread-staff = Supportteam
portal-thread-you = Jij
portal-reply-label = Antwoorden
...
portal-new-subject-placeholder = Een korte samenvatting van wat je nodig hebt
portal-new-description-label = Beschrijving
portal-new-description-placeholder = Vertel ons wat er aan de hand is en hoe we kunnen helpen.
