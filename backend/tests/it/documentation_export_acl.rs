...
fn setup(conn: &mut DbConnection, slug: &str) -> Fixture {
    let workspace_id = crate::common::mint_workspace(conn, slug, slug);
    backend::sync::session::pin_workspace(conn, workspace_id).expect("pin");
    let author = crate::common::insert_user(conn, "Doc Author");
    let reader = crate::common::insert_user(conn, "Doc Reader");
    let other = crate::common::insert_user(conn, "Someone Else");
